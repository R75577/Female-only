 ...
 * Single block (Female only) — trials randomized
...
// ======== FILENAME GENERATION =========
// Number of female identities you actually have (update if you have more)
const NUM_FACES_PER_SEX = 6;

// Height codes you have (1=Tall, 2=Average, 3=Short – keep as needed)
const HEIGHT_CODES = ['1', '2', '3'];

// *** IMPORTANT: ONLY AVERAGE/UNATTRACTIVE ATTRACTIVENESS LEVELS ***
// .2 = "less / average attractive", .3 = "unattractive"
// (NO "" baseline, so the script will NOT look for attractive images.)
const ATTR_CODES = ['.2', '.3'];

function buildFiles(sexTag /* "F.F" */) {
  const files = [];
...
function parseMeta(imgPath) {
  ...
  // examples: F.F.1_2.2.png   F.F.1_3.3.png
  ...
  const h = m[3];
  const a = m[4] || null; // '2' or '3' (or null if no suffix)

  meta.sex = (tag === 'F.F') ? 'Female' : 'Male';
  meta.face_id = face;
  meta.height_code = h;
  meta.height_label = (h === '1') ? 'Tall'
                     : (h === '2') ? 'Average'
                     : 'Short';
  meta.attract_code = a;

  // We only expect .2 and .3 in this version
  if (a === '2') {
    meta.attract_label = 'AverageUnattractive'; // you can rename if you like
  } else if (a === '3') {
    meta.attract_label = 'VeryUnattractive';
  } else {
    meta.attract_label = 'NA';
  }
...
const instructions = {
  ...
  pages: [
    `<div class="center">
       <h2>Instructions</h2>
       <p><strong>In this experiment, we will ask you to make judgments about a series of female images.</strong></p>
...
function blockIntroHTML(label) {
  const line = 'Please view and rate the following female images.';
  ...
  pages: [
    `<div class="center" style="max-width:800px;margin:0 auto;">
       ...
       <p style="margin-top:12px;">
         <a href="https://connect.cloudresearch.com/participant/project/022CBB50E6/complete"
...
timeline.push(femaleIntro, ...femaleTrials);
